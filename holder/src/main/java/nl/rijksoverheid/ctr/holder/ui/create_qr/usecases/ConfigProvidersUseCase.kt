...
class ConfigProvidersUseCaseImpl(private val coronaCheckRepository: CoronaCheckRepository) :
    ...
    override suspend fun testProviders(): TestProvidersResult {
        return when (val result = coronaCheckRepository.configProviders()) {
            is NetworkRequestResult.Success<RemoteConfigProviders> -> TestProvidersResult.Success(
                result.response.testProviders
            )
...
sealed class TestProvidersResult {
    class Success(val testProviders: List<RemoteConfigProviders.TestProvider>) :
        TestProvidersResult()
...
sealed class EventProvidersResult {
    class Success(val eventProviders: List<RemoteConfigProviders.EventProvider>) :
