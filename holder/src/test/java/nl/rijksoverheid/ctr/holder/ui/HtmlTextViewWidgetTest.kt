...
class HtmlTextViewWidgetTest : AutoCloseKoinTest() {
    ...
    fun `testHtmlParsing`() {
        ...
        assertEquals(widget.childCount, 1) // Text

        widget.setHtmlText("Paragraph 1\nParagraph 2")
        assertEquals(widget.childCount, 1) // Text

        widget.setHtmlText("<p>Paragraph 1</p>")
        assertEquals(widget.childCount, 2) // Text + Linebreak

        widget.setHtmlText("Paragraph 1<br/>Paragraph 2")
        assertEquals(widget.childCount, 2) // Text + Text

        widget.setHtmlText("<p>Paragraph 1</p><p>Paragraph 2</p>")
        assertEquals(widget.childCount, 3) // Text + Linebreak + Text
        ...
        assertEquals(widget.childCount, 5) // Heading + Text + Linebreak + Heading + Text
        ...
        assertEquals(widget.childCount, 7) // Heading + Heading + Heading + Heading + Heading + Heading + Linebreak

        // List test
        widget.setHtmlText("<ul><li>List item 1</li><li>List item 2</li></ul>")
        assertEquals(widget.childCount, 3) // Item + Item + Linebreak
