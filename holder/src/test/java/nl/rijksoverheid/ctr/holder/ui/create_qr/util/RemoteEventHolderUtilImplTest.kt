...
class RemoteEventHolderUtilImplTest: AutoCloseKoinTest() {
    ...
    fun `recovery event data are parsed correctly`() {
        ...
        val remoteUtil = RemoteEventHolderUtilImpl(moshi, mockk(), mockk(relaxed = true), YourEventsFragmentUtilImpl())
    ...
    fun `two matching holders are not conflicting`() {
        ...
        val remoteUtil = RemoteEventHolderUtilImpl(moshi, mockk(), mockk(), YourEventsFragmentUtilImpl())
