...
import nl.rijksoverheid.ctr.fakeEventProviderRepository
...
class GetEventProvidersWithTokensUseCaseImplTest {
    ...
    @Test
    fun `get() returns Success with event provider that return informationAvailable = true`() =
        runBlocking {
    ...
    @Test
    fun `get() returns Network for event provider that throws SocketTimeOutException`() =
        runBlocking {
            ...
            assertEquals(
                listOf(
                    ...
                    EventProviderWithTokenResult.Error(
                        NetworkRequestResult.Failed.ServerNetworkError(
                            HolderStep.UnomiNetworkRequest, exception
                        )
                    )
