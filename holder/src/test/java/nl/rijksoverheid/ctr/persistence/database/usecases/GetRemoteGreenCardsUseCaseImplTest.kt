...
class GetRemoteGreenCardsUseCaseImplTest : AutoCloseKoinTest() {

    private val coronaCheckRepository = mockk<CoronaCheckRepository>()
    private val mobileCoreWrapper = mockk<MobileCoreWrapper>()
    private val getRemoteProtocolFromEventGroupUseCase: GetRemoteProtocolFromEventGroupUseCase by inject()

    @Before
    fun setUp() {
        coEvery { coronaCheckRepository.getPrepareIssue() } answers {
            NetworkRequestResult.Success(
                RemotePrepareIssue("", "".toByteArray())
            )
        }

        coEvery { mobileCoreWrapper.createCommitmentMessage(any(), any()) } answers { "" }
    }

    @Test
    fun `If response returns event blocked, return BlockedEvent object`() = runTest {
        // Block event group with id 1
        ...
        val usecase = GetRemoteGreenCardsUseCaseImpl(
            coronaCheckRepository = coronaCheckRepository,
            mobileCoreWrapper = mobileCoreWrapper,
            getRemoteProtocolFromEventGroupUseCase = getRemoteProtocolFromEventGroupUseCase
        )

        // Event group with 1 one that has two events
        val firstEvent = getEvent(1)

        val secondEvent = getEvent(2)

        // Expect the events from event group 1
        val result = usecase.get(listOf(firstEvent, secondEvent), "", HolderFlow.Refresh)
        assertEquals(2, (result as RemoteGreenCardsResult.Success).blockedEvents.size)
    }

    private fun getEvent(id: Int) = EventGroupEntity(
        id = id,
        walletId = 1,
    ...
    @Test
    fun `fuzzy matched events return matching blob ids`() = runTest {
        val firstEvent = getEvent(1)
        val secondEvent = getEvent(2)
        val thirdEvent = getEvent(3)
        val fourthEvent = getEvent(4)
        ...
        val actualResult = usecase.get(listOf(firstEvent, secondEvent, thirdEvent, fourthEvent), "", HolderFlow.FuzzyMatching)

        val matchingBlobIds = (actualResult as RemoteGreenCardsResult.FuzzyMatchingError).matchingBlobIds
        assertEquals(1, matchingBlobIds[0][0])
        assertEquals(2, matchingBlobIds[0][1])
        assertEquals(3, matchingBlobIds[1][0])
        assertEquals(4, matchingBlobIds[1][1])
    }
}
