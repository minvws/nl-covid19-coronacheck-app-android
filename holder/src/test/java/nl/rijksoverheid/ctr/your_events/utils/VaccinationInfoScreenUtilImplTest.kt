...
class VaccinationInfoScreenUtilImplTest : AutoCloseKoinTest() {
    ...
    private val cachedAppConfigUseCase: HolderCachedAppConfigUseCase =
        object : HolderCachedAppConfigUseCase {
    ...
    private val vaccinationClock =
        Clock.fixed(Instant.parse("2021-08-01T00:00:00.00Z"), ZoneId.of("UTC"))

    private val event = RemoteEventVaccination(
        type = "vaccination",
        unique = "unique",
    ...
    fun `get correct vaccination screen info if has european credential`() {
        ...
        assertEquals("Details", infoScreen.title)
        assertEquals(
            "De volgende gegevens staan in je bewijs:<br/><br/>Naam: <b>Surname</b><br/>Geboortedatum: <b>01-08-1982</b><br/><br/>Ziekteverwekker: <b>COVID-19</b><br/>Vaccin: <b>Pfizer (Comirnaty)</b><br/>Productnaam: <b>PFIZER INJVLST 0,3ML</b><br/>Type vaccin: <b>type</b><br/>Vaccinproducent: <b>Biontech Manufacturing GmbH</b><br/>Dosis: <b>1 / 2</b><br/>Vaccinatiedatum: <b>1 augustus 2021</b><br/>Gevaccineerd in: <b>Nederland</b><br/>Afgever certificaat: <b>Bart</b><br/><br/>Uniek vaccinatienummer: <b>unique</b><br/>Footer Text",
            infoScreen.description
        )
    }

    @Test
    fun `get correct issuer if has dcc credential`() {
        val context = ApplicationProvider.getApplicationContext<Context>()
        val resources = context.resources

        val paperProofUtil: PaperProofUtil = mockk()
        every { paperProofUtil.getIssuer(any()) } answers { "Ministry of Health Welfare and Sport" }
        ...
        assertTrue(infoScreen.description.contains("Ministerie van VWS / Ministry of Health, Welfare and Sport"))
    }
}
